...
function StatPill({ label, value, sub, icon, color }) {
  return (
    <div style={{ background: "#f6f6f6", borderRadius: 10, padding: "14px 16px", flex: "1 1 120px", minWidth: 120, fontFamily: F }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 6, minWidth: data.length * 64, height: barH + 40, paddingTop: 20 }}>
        {data.map(d => { const h = maxVal > 0 ? (d.value / maxVal) * barH : 0; return (
          <div key={d.name} style={{ flex: 1, minWidth: 48, maxWidth: 80, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", height: barH + 40 }}>
            <span style={{ fontSize: 10, fontFamily: F, fontWeight: 700, marginBottom: 4, color: d.color }}>{fmt(d.value)}</span>
            ...
            <span style={{ fontSize: 8, fontFamily: F, marginTop: 6, textAlign: "center", color: "#666", maxWidth: 80, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", width: "100%" }}>{d.name}</span>
          </div>
        ); })}
      </div>
    </div>
...
function UploadPage({ onFile, fileRef, loading, error, debugInfo }) {
  ...
  return (
    <div style={{ maxWidth: 420, margin: "0 auto", padding: "60px 24px", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: F }}>
      <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;500;600;700;800&display=swap" rel="stylesheet" />
      <div style={{ marginBottom: 32, textAlign: "center" }}>
        <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: "-0.02em", marginBottom: 4 }}><span style={{ color: "#6739B7" }}>📱</span> PhonePe Tracker</div>
      ...
      {/* Privacy note */}
      <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 8, padding: "10px 16px", marginBottom: 24, textAlign: "center", fontSize: 11, color: "#166534", fontWeight: 600, lineHeight: 1.5, width: "100%", maxWidth: 340 }}>
        🔒 100% offline — your data never leaves your device. No database, no tracking, no servers.
      </div>
          ...
          <div style={{ background: "#f6f6f6", borderRadius: 10, padding: "16px 20px", textAlign: "left" }}>
            {steps.map((s, i) => (
              <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "10px 0", borderBottom: i < steps.length - 1 ? "1px solid #eee" : "none" }}>
                <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#000", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, flexShrink: 0 }}>{s.num}</div>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#333", paddingTop: 4 }}>{s.text}</div>
              </div>
            ))}
          </div>
        </div>
...
function StaticSidebar({ page, setPage, txns, fileRef, onFile }) {
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: SW, zIndex: 100, background: "#fff", borderRight: "1px solid #eee", display: "flex", flexDirection: "column", fontFamily: F, overflowY: "auto" }}>
      <div style={{ padding: "18px 16px 12px", borderBottom: "1px solid #eee" }}>
        <div style={{ fontSize: 16, fontWeight: 800, letterSpacing: "-0.02em", marginBottom: 10 }}><span style={{ color: "#6739B7" }}>📱</span> PhonePe Tracker</div>
        <div style={{ display: "flex", gap: 12, fontSize: 11 }}>
          <div><div style={{ fontSize: 13, fontWeight: 700, color: "#E63946" }}>{fmt(totalSpent)}</div><div style={{ color: "#999", fontSize: 9, textTransform: "uppercase", letterSpacing: "0.08em" }}>Spent</div></div>
          <div><div style={{ fontSize: 13, fontWeight: 700, color: "#2A9D8F" }}>{fmt(totalReceived)}</div><div style={{ color: "#999", fontSize: 9, textTransform: "uppercase", letterSpacing: "0.08em" }}>Received</div></div>
        </div>
      </div>
      <div style={{ flex: 1, padding: "6px 0" }}>
        {items.map(i => { const active = page === i.key; return (
          <button key={i.key} onClick={() => setPage(i.key)} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "11px 18px", border: "none", cursor: "pointer", background: active ? "#f0f0f0" : "transparent", color: "#000", fontSize: 13, fontWeight: active ? 700 : 500, fontFamily: F, textAlign: "left", transition: "background 0.15s ease", borderLeft: active ? "3px solid #000" : "3px solid transparent" }}>
            <span style={{ fontSize: 15 }}>{i.icon}</span><span>{i.label}</span>
          </button>
        ); })}
      </div>
      {/* Privacy badge */}
      <div style={{ padding: "8px 16px", fontSize: 9, color: "#2A9D8F", fontWeight: 600, textAlign: "center", lineHeight: 1.4 }}>🔒 100% offline · no data leaves your device</div>
      <div style={{ padding: "10px 16px", borderTop: "1px solid #eee" }}>
        <button onClick={() => fileRef.current?.click()} style={{ width: "100%", padding: "8px 0", border: "2px solid #000", background: "#000", color: "#fff", fontSize: 10, fontFamily: F, fontWeight: 700, cursor: "pointer", letterSpacing: "0.06em", textTransform: "uppercase", borderRadius: 4 }}>Upload New PDF</button>
        <input ref={fileRef} type="file" accept=".pdf" onChange={onFile} style={{ display: "none" }} />
      </div>
...
function ThresholdAnalysis({ txns, mode, setMode, threshold, setThreshold, inputVal, setInputVal }) {
  ...
  return (
    <div style={{ background: "#f6f6f6", borderRadius: 10, padding: "16px 18px", fontFamily: F }}>
      ...
      <div style={{ background: "#fff", borderRadius: 8, padding: "12px 14px", border: "1px solid #eee", textAlign: "center" }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: "#333" }}>
          ...
          <span style={{ fontSize: 22, fontWeight: 800 }}>{thresholdDays}</span> <span style={{ color: "#999" }}>out of {totalDays} days</span>
        </span>
...
function OverviewPage({ txns, categoryData, totalSpent, totalReceived, dailyData, weeklyData, topMerchants, avgDaily, topTxn, dateRange }) {
  ...
  const debits = txns.filter(t => t.type === "DEBIT");
  const credits = txns.filter(t => t.type === "CREDIT");
  const ttStyle = { background: "#fff", border: "1px solid #eee", borderRadius: 8, fontSize: 12, color: "#000", fontFamily: F };
  ...
  return (
    <div>
      <div style={{ background: "#f6f6f6", borderRadius: 10, padding: "10px 14px", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between", fontFamily: F, fontSize: 12, fontWeight: 700, color: "#666" }}>
      ...
      <SH>Extremes</SH>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontFamily: F }}>
        {[
          ...
          <div key={e.label} style={{ background: e.bg, borderRadius: 8, padding: "12px 14px", border: `1px solid ${e.border}` }}>
            <div style={{ fontSize: 9, color: e.color, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>{e.label}</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: e.color }}>{e.data ? fmt(e.data.amount) : "—"}</div>
            {e.data && <div style={{ fontSize: 10, color: "#999", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{e.data.detail}</div>}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 20, background: "#f6f6f6", borderRadius: 10, padding: 18 }}>
        <ResponsiveContainer width="48%" height={170}><PieChart><Pie data={categoryData} dataKey="value" cx="50%" cy="50%" innerRadius={38} outerRadius={70} strokeWidth={2} stroke="#fff">{categoryData.map((d, i) => <Cell key={i} fill={d.color} />)}</Pie><Tooltip contentStyle={ttStyle} formatter={v => fmt(v)} /></PieChart></ResponsiveContainer>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 5, fontFamily: F }}>
          {categoryData.slice(0, 7).map(d => (<div key={d.name} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11 }}><div style={{ width: 7, height: 7, borderRadius: 2, background: d.color, flexShrink: 0 }} /><span style={{ color: "#666", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.name}</span><span style={{ fontWeight: 700, fontSize: 10 }}>{d.pct.toFixed(0)}%</span></div>))}
        </div>
      </div>
      <SH>Daily Spending</SH>
      <div style={{ background: "#f6f6f6", borderRadius: 10, padding: "14px 6px" }}>
        <ResponsiveContainer width="100%" height={200}><AreaChart data={dailyData}><defs><linearGradient id="gS" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#000" stopOpacity={0.15} /><stop offset="95%" stopColor="#000" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" /><XAxis dataKey="date" tick={{ fontSize: 9, fill: "#999", fontFamily: F }} tickFormatter={v => { const d = new Date(v); return `${d.getDate()}/${d.getMonth()+1}`; }} /><YAxis tick={{ fontSize: 9, fill: "#999", fontFamily: F }} tickFormatter={v => `₹${(v/1000).toFixed(0)}k`} /><Tooltip contentStyle={ttStyle} formatter={v => fmt(v)} /><Area type="monotone" dataKey="spent" stroke="#000" fill="url(#gS)" strokeWidth={2} name="Spent" /><Area type="monotone" dataKey="received" stroke="#2A9D8F" fill="none" strokeWidth={2} strokeDasharray="5 5" name="Received" /><Legend wrapperStyle={{ fontSize: 10, fontFamily: F }} /></AreaChart></ResponsiveContainer>
      </div>
      <SH>Top Merchants</SH>
      {topMerchants.map((m, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "1px solid #f0f0f0", fontFamily: F, fontSize: 13 }}><span style={{ width: 22, height: 22, borderRadius: 5, background: "#f0f0f0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, flexShrink: 0, color: "#666" }}>{i + 1}</span><span style={{ flex: 1, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.name}</span><span style={{ color: "#999", fontSize: 11 }}>{m.count}x</span><span style={{ fontWeight: 700, fontSize: 12 }}>{fmt(m.total)}</span></div>))}
      <SH>Weekly Spending</SH>
      <div style={{ background: "#f6f6f6", borderRadius: 10, padding: "14px 6px" }}>
        <ResponsiveContainer width="100%" height={180}><BarChart data={weeklyData}><CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" /><XAxis dataKey="week" tick={{ fontSize: 9, fill: "#999", fontFamily: F }} /><YAxis tick={{ fontSize: 9, fill: "#999", fontFamily: F }} tickFormatter={v => `₹${(v/1000).toFixed(0)}k`} /><Tooltip contentStyle={{ background: "#fff", border: "1px solid #eee", borderRadius: 8, fontSize: 12, fontFamily: F }} formatter={v => fmt(v)} /><Bar dataKey="spent" fill="#000" radius={[4, 4, 0, 0]} name="Spent" /><Bar dataKey="received" fill="#2A9D8F" radius={[4, 4, 0, 0]} name="Received" /></BarChart></ResponsiveContainer>
      </div>
    </div>
  );
}
...
function TransactionsPage({ txns, setTxns }) {
  ...
  const filtered = useMemo(() => {
    ...
    list = [...list].sort((a, b) => { let c = 0; if (sortBy === "date") c = a.dateObj - b.dateObj; else if (sortBy === "amount") c = a.amount - b.amount; else if (sortBy === "category") c = a.category.localeCompare(b.category); else if (sortBy === "type") c = a.type.localeCompare(b.type); return sortDir === "desc" ? -c : c; });
    return list;
  ...
  const hS = { fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "#999", cursor: "pointer", display: "flex", alignItems: "center", gap: 3, userSelect: "none" };
  return (
    <div>
      <SH>Transactions ({filtered.length})</SH>
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
        <input placeholder="Search..." value={searchQ} onChange={e => setSearchQ(e.target.value)} style={{ border: "2px solid #000", padding: "8px 12px", fontSize: 12, fontFamily: F, flex: 1, minWidth: 120, background: "transparent", outline: "none", fontWeight: 600 }} />
        <select value={catFilter} onChange={e => setCatFilter(e.target.value)} style={{ border: "2px solid #000", padding: "8px 10px", fontSize: 12, fontFamily: F, fontWeight: 600, background: "#fff", outline: "none", cursor: "pointer" }}>{categories.map(c => <option key={c} value={c}>{c}</option>)}</select>
        <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} style={{ border: "2px solid #000", padding: "8px 10px", fontSize: 12, fontFamily: F, fontWeight: 600, background: "#fff", outline: "none", cursor: "pointer" }}><option value="All">All</option><option value="DEBIT">Debits</option><option value="CREDIT">Credits</option></select>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "74px 1fr 100px 50px 80px 26px", padding: "8px 0", borderBottom: "2px solid #000", fontFamily: F }}>
        <span style={hS} onClick={() => toggleSort("date")}>Date {sortIcon("date")}</span><span style={{ ...hS, cursor: "default" }}>Details</span><span style={hS} onClick={() => toggleSort("category")}>Category {sortIcon("category")}</span><span style={hS} onClick={() => toggleSort("type")}>Type {sortIcon("type")}</span><span style={{ ...hS, justifyContent: "flex-end" }} onClick={() => toggleSort("amount")}>Amount {sortIcon("amount")}</span><span></span>
      </div>
      ...
      <div style={{ maxHeight: "calc(100vh - 220px)", overflowY: "auto" }}>
        {filtered.map(t => (
          <div key={t.id} style={{ display: "grid", gridTemplateColumns: "74px 1fr 100px 50px 80px 26px", padding: "9px 0", borderBottom: "1px solid #f0f0f0", fontFamily: F, fontSize: 12, alignItems: "center" }}>
            ...
            <span>{editingId === t.id ? (<div style={{ display: "flex", gap: 3, alignItems: "center" }}><select value={editCat} onChange={e => setEditCat(e.target.value)} style={{ border: "1px solid #ccc", padding: "2px 3px", fontSize: 9, fontFamily: F, outline: "none", maxWidth: 72 }}>{ALL_CATEGORY_NAMES.map(c => <option key={c} value={c}>{c}</option>)}</select><button onClick={() => saveCategory(t.id)} style={{ border: "none", background: "#000", color: "#fff", padding: "2px 5px", fontSize: 8, fontFamily: F, fontWeight: 700, cursor: "pointer", borderRadius: 2 }}>✓</button></div>) : (<span onClick={() => { setEditingId(t.id); setEditCat(t.category); }} style={{ fontSize: 9, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 3, color: getCatColor(t.category), fontWeight: 600 }} title="Click to change"><span style={{ width: 5, height: 5, borderRadius: 2, background: getCatColor(t.category), display: "inline-block" }} />{t.category}</span>)}</span>
...
function TrendsPage({ txns, dailyData, categoryData, topMerchants, topTxn }) {
  ...
  const weekAnalysis = useMemo(() => {
    ...
    if (daysInMonth > 28) ranges.push({ label: "Week 5", sub: `29–${daysInMonth}`, start: 29, end: daysInMonth });
    ...
    return ranges.map(r => {
      ...
      for (let d = r.start; d <= Math.min(r.end, daysInMonth); d++) {
        ...
        // Also try matching with month name format from parsed dates
        ...
        // Also search with the date string format from parsed dates
        const altKey = dayObj.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }).replace(",", ",");
        const matchedAmt = amt || Object.entries(dailyMap).find(([k]) => {
  ...
  const tCell = { padding: "8px 10px", fontSize: 12, fontFamily: F, borderBottom: "1px solid #eee" };
  const tHdr = { ...tCell, fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "#999", background: "#f6f6f6", borderBottom: "2px solid #000" };

  return (
    <div>
      {/* Weekday vs Weekend Analysis */}
      <SH>Weekday vs Weekend (Fri-Sun) Spending</SH>
      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        <StatPill label="Weekday Total" value={fmt(totalWeekday)} color="#457B9D" />
        <StatPill label="Weekend Total" value={fmt(totalWeekend)} color="#E63946" />
      </div>
      <div style={{ borderRadius: 10, overflow: "hidden", border: "1px solid #eee", marginBottom: 32 }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: F }}>
          <thead>
            <tr>
              <td style={tHdr}>Week</td><td style={tHdr}>Days</td><td style={{ ...tHdr, textAlign: "right" }}>Total</td><td style={{ ...tHdr, textAlign: "right", color: "#457B9D" }}>Weekday</td><td style={{ ...tHdr, textAlign: "right", color: "#E63946" }}>Weekend</td>
            </tr>
          </thead>
          <tbody>
            {weekAnalysis.map((w, i) => (
              <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                ...
                <td style={{ ...tCell, color: "#999", fontSize: 11 }}>{w.sub}</td>
                <td style={{ ...tCell, textAlign: "right", fontWeight: 700 }}>{fmt(w.total)}</td>
                <td style={{ ...tCell, textAlign: "right", color: "#457B9D", fontWeight: 600 }}>{fmt(w.weekdayTotal)}<br/><span style={{ fontSize: 9, color: "#999" }}>{w.weekdayCount}d</span></td>
                <td style={{ ...tCell, textAlign: "right", color: "#E63946", fontWeight: 600 }}>{fmt(w.weekendTotal)}<br/><span style={{ fontSize: 9, color: "#999" }}>{w.weekendCount}d</span></td>
              </tr>
            ))}
      ...
      <div style={{ background: "#f6f6f6", borderRadius: 10, padding: "14px 6px" }}>
        <ResponsiveContainer width="100%" height={200}><AreaChart data={(() => { let c = 0; return dailyData.map(d => { c += d.spent; return { ...d, cumulative: c }; }); })()}><defs><linearGradient id="gC" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#E63946" stopOpacity={0.15} /><stop offset="95%" stopColor="#E63946" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" /><XAxis dataKey="date" tick={{ fontSize: 9, fill: "#999", fontFamily: F }} tickFormatter={v => { const d = new Date(v); return `${d.getDate()}/${d.getMonth()+1}`; }} /><YAxis tick={{ fontSize: 9, fill: "#999", fontFamily: F }} tickFormatter={v => `₹${(v/1000).toFixed(0)}k`} /><Tooltip contentStyle={ttStyle} formatter={v => fmt(v)} /><Area type="monotone" dataKey="cumulative" stroke="#E63946" fill="url(#gC)" strokeWidth={2.5} name="Total Spent" /></AreaChart></ResponsiveContainer>
      </div>
      <SH>By Day of Week</SH>
      <div style={{ background: "#f6f6f6", borderRadius: 10, padding: "14px 6px" }}>
        <ResponsiveContainer width="100%" height={180}><BarChart data={(() => { const d = ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(x => ({ day: x, amount: 0 })); debits.forEach(t => d[t.dateObj.getDay()].amount += t.amount); return d; })()}><CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" /><XAxis dataKey="day" tick={{ fontSize: 9, fill: "#999", fontFamily: F }} /><YAxis tick={{ fontSize: 9, fill: "#999", fontFamily: F }} tickFormatter={v => `₹${(v/1000).toFixed(0)}k`} /><Tooltip contentStyle={ttStyle} formatter={v => fmt(v)} /><Bar dataKey="amount" fill="#000" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
      </div>
      <SH>By Time of Day</SH>
      <div style={{ background: "#f6f6f6", borderRadius: 10, padding: "14px 6px" }}>
        <ResponsiveContainer width="100%" height={180}><BarChart data={(() => { const s = [{slot:"Morning",amount:0},{slot:"Afternoon",amount:0},{slot:"Evening",amount:0},{slot:"Night",amount:0}]; debits.forEach(t => { const h=t.dateObj.getHours(); if(h>=6&&h<12)s[0].amount+=t.amount; else if(h>=12&&h<17)s[1].amount+=t.amount; else if(h>=17&&h<21)s[2].amount+=t.amount; else s[3].amount+=t.amount; }); return s; })()}><CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" /><XAxis dataKey="slot" tick={{ fontSize: 9, fill: "#999", fontFamily: F }} /><YAxis tick={{ fontSize: 9, fill: "#999", fontFamily: F }} tickFormatter={v => `₹${(v/1000).toFixed(0)}k`} /><Tooltip contentStyle={ttStyle} formatter={v => fmt(v)} /><Bar dataKey="amount" fill="#2A9D8F" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontFamily: F }}>
        ...
        ].map((ins, i) => (<div key={i} style={{ background: "#f6f6f6", borderRadius: 10, padding: "12px 14px" }}><div style={{ fontSize: 16, marginBottom: 3 }}>{ins.icon}</div><div style={{ fontSize: 9, color: "#999", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>{ins.title}</div><div style={{ fontSize: 12, fontWeight: 700 }}>{ins.value}</div></div>))}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#fff", color: "#000", fontFamily: F }}>
      <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;500;600;700;800&display=swap" rel="stylesheet" />
